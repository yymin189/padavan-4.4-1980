...
<html>
	<head>
		...
		<script type="text/javascript" src="/disk_folder_tree.js">
		</script>
		<script>
...
function initial() {
    show_banner(1);
    show_menu('if-m1-syssettings', 'if-m2-usbapp', 2);
    show_footer();
...
function showAccountMenu() {
    ...
    if (this.accounts.length <= 0)
        ...
        for (var i = 0; i < this.accounts.length; ++i) {
            account_menu_code += '<i class="if if-ftp-user" id="';
...
function contrastSelectAccount(selectedObj) {
    if (this.lastClickedAccount != 0) {
        $j(this.lastClickedAccount).removeClass('if-color-red');
    }

    $j(this.selectedObj).addClass('if-color-red');
...
function changeActionButton(selectedObj, type, action, flag) {
    if (type == "User" && this.accounts.length <= 0) {
        if (action == "Del" || action == "Mod")
            return;
    }

    $j(selectedObj).removeClass('enabled clicked');
    if (typeof(flag) == "number") {
        if (flag == 0) $j(selectedObj).addClass('enabled');
        if (flag == 1) $j(selectedObj).addClass('clicked');
    }
}
...
function unload_body() {
    ...
    $("modifyFolderBtn").onmouseout = function() {};
		...
		<style>
			.FdTemp th, .FdTemp td, .t-border-top-none th, .t-border-top-none td {
			border-top: 0 none; margin-bottom: 0px; vertical-align: top; } .FdTemp
			table td { padding: 0px 0px 0px 0px; } .nav-tabs>li>a { padding-right:
			6px; padding-left: 6px; }
		</style>
	</head>
	<body onLoad="initial();" onunload="unload_body();">
		...
		<div class="wrapper">
									...
									<div class="box well grad_colour_dark_blue">
										...
										<div class="round_bottom">
											<div class="row-fluid">
												...
												<!--table cellpadding="4" cellspacing="0" class="table" style="margin-bottom:
												0px;">
												</table-->
												<!-- The table of share. -->
												<div id="shareStatus">
													...
													<table cellpadding="2" cellspacing="0" class="table t-border-top-none"
													style="margin-bottom: 0px; border-top: 0 none;">
														<tr>
															<!-- The action buttons of accounts. -->
															<td width="35%" style="vertical-align: top;">
																<i id="createAccountBtn" class="if if-ftp-user-add" hspace="1" title="<#AddAccountTitle#>">
																</i>
																<i id="deleteAccountBtn" class="if if-ftp-user-del" hspace="1" title="<#DelAccountTitle#>">
																</i>
																<i id="modifyAccountBtn" class="if if-ftp-user-edit" hspace="1" title="<#ModAccountTitle#>">
																</i>
															</td>
															<!-- The action buttons of folders. -->
															<td>
																<i id="createFolderBtn" class="if if-ftp-folder-add" hspace="1" title="<#AddFolderTitle#>">
																</i>
																<i id="deleteFolderBtn" class="if if-ftp-folder-del" hspace="1" title="<#DelFolderTitle#>">
																</i>
																<i id="modifyFolderBtn" class="if if-ftp-folder-edit" hspace="1" title="<#ModFolderTitle#>">
																</i>
															</td>
														</tr>
													</table>
												...
												<table cellpadding="0" cellspacing="1" class="table" style="height: 500px;">
													<tr>
														...
														<td style="vertical-align: top;">
															<table cellspacing="0" cellpadding="0" class="table t-border-top-none"
															style="margin-bottom: 0px; border-top: 0 none;">
																<tr>
																	<td width="220" align="left" style="padding-left: 0px; padding-top: 0px; padding-bottom: 0px;">
																		<i class="if if-device-router">
																		</i>
																		<span>
																			<% nvram_get_x( "", "productid"); %>
																		</span>
																	</td>
																	<td style="padding-top: 0px; padding-bottom: 0px;">
																		<div id="permissionTitle">
																		</div>
																	</td>
																</tr>
															</table>
															<!-- the tree of folders -->
															<div id="e0" class="FdTemp" style="font-size:9pt; margin-top:1px;">
															</div>
															<div style="text-align:right; margin:10px auto; border-top:1px dotted #CCC; width:95%; padding:2px;">
																<center>
																	<input name="changePermissionBtn" id="changePermissionBtn" type="button"
																	value="<#CTL_apply#>" class="btn btn-primary" style="width: 219px" disabled="disabled">
																</center>
															</div>
														</td>
													</tr>
			...
			<div id="OverlayMask" class="popup_bg">
				<div align="center">
					<iframe src="" frameborder="0" scrolling="no" id="popupframe" width="400"
					height="400" allowtransparency="true" style="margin-top:150px;">
					</iframe>
				</div>
			</div>
		</div>
