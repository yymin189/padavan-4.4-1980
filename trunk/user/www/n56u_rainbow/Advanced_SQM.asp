  ...
  <head>
    ...
    <script>
      ...
      function fill_sqm_status(status_code) {
        var stext = "Unknown";
        if (status_code == 0) stext = "<#Running#>";
        else stext = "<#Stopped#>";
      ...
      function showTab(curHash) {
        ...
        if (obj == null || obj.style.display == 'none') curHash = '#cfg';
      ...
      function getHash() {
        ...
        for (var i = 0; i < arrHashes.length; i++) {
          if (curHash == ('#' + arrHashes[i])) return curHash;
        }
        return ('#' + arrHashes[0]);
      }
              ...
              <div class="row-fluid">
                <div class="span12">
                  <div class="box well grad_colour_dark_blue">
                    <h2 class="box_head round_top">SQM QoS -
                      <#menu5_37#></h2>
                    <div class="round_bottom">
                      <div>
                        <ul class="nav nav-tabs" style="margin-bottom: 10px;">
                          <li class="active">
                            <a href="Advanced_SQM.asp">SQM QOS</a></li>
                        </ul>
                      </div>
                      <div class="row-fluid">
                      ...
                      <table width="100%" align="center" cellpadding="4" cellspacing="0" class="table" style="margin-top: 10px;">
                        <tr style="display:none;">
                          ...
                          <td colspan="2">
                            ...
                            <span class="label label-info" style="margin-left: 0px;">
                              <% nvram_get_x( "", "sqm_qdisc"); %></span>
                            <span class="label label-info" style="margin-left: 20px;">
                              <% nvram_get_x( "", "sqm_script"); %></span>
                          </td>
                        </tr>
                        <tr>
                          <th>启用SQM</th>
                          <td colspan="2">
                            <input type="checkbox" id="sqm_enable_fake" <% nvram_match_x( "", "sqm_enable", "1", "value=1 checked"); %>
                            <% nvram_match_x( "", "sqm_enable", "0", "value=0"); %>/>
                              <div style="position: absolute; margin-left: -10000px;">
                                ...
                                <input type="radio" value="0" name="sqm_enable" id="sqm_enable_0" class="input" value="0" <% nvram_match_x( "", "sqm_enable", "0", "checked"); %>/> No</div></td>
                        </tr>
                        <tr>
                          <th>流控对象</th>
                          <td>
                        ...
                        <tr>
                          ...
                          <td>
                            <select name="sqm_qdisc" class="input">
                              ...
                              <option value="cake" <% nvram_match_x( "", "sqm_qdisc", "cake", "selected"); %>>cake</option></select>
                          </td>
                        </tr>
                        <tr>
                          <th>队列脚本</th>
                          <td>
                            <select name="sqm_script" class="input">
                              ...
                              <option value="layer_cake.qos" <% nvram_match_x( "", "sqm_script", "layer_cake.qos", "selected"); %>>layer_cake</option></select>
                          </td>
                        </tr>
                        <tr>
                          <th width="32%">下载速度 (
                            <span class="label label-info">kbit/s</span>)</th>
                          <td>
                        ...
                        <tr>
                          <th width="32%">上传速度 (
                            <span class="label label-info">kbit/s</span>)</th>
                          <td>
                            <input type="text" maxlength="10" class="input" size="10" id="sqm_up_speed" name="sqm_up_speed" value="<% nvram_get_x(" ","sqm_up_speed "); %>"/></td>
                          <td>&nbsp;
                            <span style="color:#888;">测速的80-95％，1 Mbps = 1024 kbit/s 需填写小于下载速度的值</span></td>
                        </tr>
                        <tr>
                          <th>启用日志</th>
                          <td>
                            <input type="checkbox" id="sqm_debug_log_fake" <% nvram_match_x( "", "sqm_debug_log", "1", "value=1 checked"); %>
                            <% nvram_match_x( "", "sqm_debug_log", "0", "value=0"); %>/>
                              <div style="position: absolute; margin-left: -10000px;">
                                ...
                                <input type="radio" value="0" name="sqm_debug_log" id="sqm_debug_log_0" class="input" value="0" <% nvram_match_x( "", "sqm_debug_log", "0", "checked"); %>/> No</div></td>
                          <td>&nbsp;
                            <span style="color:#888;">/var/run/sqm/
                              <% nvram_get_x( "", "sqm_active"); %>.debug.log</span></td>
                        </tr>
                        <tr>
                          <th>日志等级</th>
                          <td>
                            <select name="sqm_log_level" class="input">
                              ...
                              <option value="10" <% nvram_match_x( "", "sqm_log_level", "10", "selected"); %>>trace</option></select>
                          </td>
                          <td>&nbsp;
                            <span style="color:#888;">仅调试时启用debug/trace</span></td>
                        </tr>
                      </table>
                      <table class="table">
                        <tr>
                          <td colspan="3" style="border-top: 0 none;">
                            <br/>
                            <center>
                              <input class="btn btn-primary" style="width: 219px" type="button" value="<#CTL_apply#>" onclick="applyRule()" /></center>
                          </td>
                        </tr>
                      </table>
                    </div>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title">SQM QoS 速度计算器</h4></div>
          <div class="modal-body">
            <div>
              <span style="display: inline-block; width:68px;">带宽</span>:
              <input type="text" class="span2" style="margin: 1px 5px;" id="bw_in_Mbps" value="10">Mbps</div>
            <div>
              <span style="display: inline-block; width:68px;">百分比</span>:
              <input type="text" class="span2" style="margin: 1px 5px;" id="bw_percent" value="95">%</div>
            <div>
              <span style="display: inline-block; width:68px;">结果</span>:
              <input type="text" readonly class="span2" style="margin: 1px 5px;" id="bw_result" value="">kbit/s</div></div>
          <div class="modal-footer">
            <button type="button" id="bw_set_down_action" class="btn btn-primary">设置下载速度</button>
            <button type="button" id="bw_set_up_action" class="btn btn-primary">设置上传速度</button></div>
        </div>
      </div>
    </div>
    <script>(function($) {
