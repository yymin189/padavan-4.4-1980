...
<html>
	<head>
		...
		<script type="text/javascript" src="/bootstrap/js/jquery.xdomainajax.js">
		</script>
		<script type="text/javascript" src="/bootstrap/js/jquery.maskedinput-1.3.min.js">
		</script>
...
function sendWakeUp(mac, $button) {
    ...
    if (mac != '') {
        $j.getJSON('/wol_action.asp', {
                dstmac: mac
            },
            function(response) {
                var respMac = (response != null && typeof response === 'object' && "wol_mac" in response) ? response.wol_mac.toUpperCase() : null;
...
$j(document).ready(function() {
    ...
    $j('#wol_btn, .btn_wakeup').click(function() {
        ...
        mac = ($button.prop('id') == 'wol_btn') ? $button.parents('tr').find('.mac').val() : mac = $button.parents('tr').find('.mac').text();
		...
		<style>
			#wol_table{ margin-top: 20px; } #wol_table td:first-child { font-family:
			monospace; font-weight: bold; } .wol_response { display: none; float: left;
			padding: 5px 11px; width: 52px; height: 18px; text-align: center; border-radius:
			5px; }
		</style>
	</head>
	<body onload="initial();">
												...
												<table width="100%" cellpadding="4" cellspacing="0" class="table" style="margin-top: 10px;">
													<tr>
														...
														<td width="395px" style="border-top: 0 none; ">
															<input style="float: left; margin-right: 5px; font-family: monospace"
															id="wol_mac" type="text" maxlength="17" class="span12 mac" size="15" name="wol_mac"
															value="<% nvram_get_x(" ","wol_mac_last "); %>"/>
														</td>
														<td style="border-top: 0 none; ">
															<input type="button" id="wol_btn" class="btn btn-primary" value="<#WOL_Wake_up#>"
															/>
															<div class="wol_response" class="alert">
															</div>
														</td>
													</tr>
												</table>
